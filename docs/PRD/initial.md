...
**布局设计**：采用左右分栏布局，左侧为列表+筛选区，右侧为详情展示区。
...
- **JTBD 3 (状态管理)**：当我想要标记一个灵感已处理时，我可以点击状态图标将其从 TODO 切换为 DONE，以便于区分已处理和待处理的内容。
- **JTBD 4 (筛选与搜索)**：当我想要快速找到某个灵感时，我可以使用筛选功能按状态（All/TODO/DONE）、时间（Today/Week/Month）进行过滤，或通过搜索框快速定位内容。
- **JTBD 5 (详情编辑)**：当我想要补充或修改某个灵感的内容时，我可以在右侧详情面板中直接编辑，系统会自动保存我的修改。
- **JTBD 6 (转换为 Issue)**：当我想要将一个灵感转化为可执行的任务时，我可以点击"转为 Issue"按钮，在弹出的对话框中设置标题、优先级、类型和所属项目，并选择是否删除原项。
- **JTBD 7 (转换为 Project)**：当我想要将一个灵感转化为一个新项目时，我可以点击"转为 Project"按钮，在弹出的对话框中设置项目名称和描述，并选择是否删除原项。
- **JTBD 8 (AI 自动分类 - V2规划)**：当我积攒了大量乱七八糟的 Inbox 草稿时，我想要一键调用 AI 帮我自动打标签（如：#灵感、#待回邮件、#今日待办），以便于我能快速过滤。
