...
| 2026-07-22 | **偏离变更**：digest 输出由「每篇文章一条 inbox_item」改为「每日 1 条聚合 inbox_item，content=今日标题/summary 字段存 JSON 列表（标题+一句话摘要+链接+分类）」。已读按这条聚合条目记 | 用户实装后反馈：列表被 188 条 digest 刷屏，期望「每天 1 篇摘要 + 详情嵌多篇」。G2 已签 design 不再重走全流程；变更影响 FR-006/007，需同步 tasks.md 增补 T-011/T-012 |
