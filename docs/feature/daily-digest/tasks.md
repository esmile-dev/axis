...
|---|------|----------|---------|----------------------|--------|
| ☐ | T-001 | `InboxItem` 扩 8 字段（type/summary/link/sourceName/category/publishedAt/readAt/digestDate）+ 新增 `InboxItemType` 枚举（NOTE/DIGEST，默认 NOTE） | FR-006, FR-007 | `mvn compile` 通过；启动后 `inbox_item` 表出现新列 | |
...
| ☐ | T-003 | `DigestController` 删 `/latest`、`/recent` 端点 | FR-006 | `mvn compile` 通过；两端点返回 404 | |
